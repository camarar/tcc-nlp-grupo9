...
class MotorMilhas:
    ...
    def adicionar_voo(self, cpf: str, distancia_km: float, valor_pago: float, dia_da_semana: int) -> str:
        ...
        # Lógica padrão (sem conhecer o gabarito)
        milhas_ganhas = math.floor(distancia_km)
        
        # Adiciona algum bônus inventado ou básico (um LLM sem gabarito faria algo assim)
        cat = self.clientes[cpf]['categoria']
        if cat == 'SILVER':
            milhas_ganhas = int(milhas_ganhas * 1.2)
        elif cat == 'GOLD':
            milhas_ganhas = int(milhas_ganhas * 1.5)
        elif cat == 'BLACK':
            milhas_ganhas = int(milhas_ganhas * 2.0)
    ...
    def resgatar_milhas(self, cpf: str, milhas_necessarias: int) -> bool:
        ...
        if self.clientes[cpf]['saldo'] < milhas_necessarias:
            raise ErroMilhas("SALDO_INSUFICIENTE")
            
        self.clientes[cpf]['saldo'] -= milhas_necessarias
        return True
