"""
Agente Working Backwards Designer — Concepção e Design de Soluções interativo via terminal.

Uso:
    py working_backwards_designer.py
    py working_backwards_designer.py --file caminho/para/ideia.md
"""
...
PROMPT_FILE = SCRIPT_DIR / "Agente Working Backwards Designer.md"
META_FILE = SCRIPT_DIR / "working-backwards-designer.meta.json"
...
TOOLS = [
    {
        "name": "read_file",
        "description": (
            "Lê o conteúdo de um arquivo do sistema de arquivos local para servir de contexto. "
            "Use quando o usuário fornecer um caminho de arquivo de ideias ou requisitos brutos."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "list_directory",
        "description": "Lista arquivos de ideias ou documentação em um diretório.",
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {
                ...
                "extensions": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Extensões para filtrar, ex: ['.md', '.txt']. Omita para listar tudo."
                }
            },
            "required": ["path"]
        }
...
def load_system_prompt() -> str:
    if not PROMPT_FILE.exists():
        ...
        return "Você é um arquiteto de soluções focado em Working Backwards."
...
def print_response(text: str) -> None:
    ...
    print(fmt("  CONCEPÇÃO / DESIGN", C.BOLD + C.GREEN))
    print(fmt(SEPARATOR, C.DIM))
    print()

    tag_colors = {
        "[SUGESTÃO — premissa adotada]": C.YELLOW + C.BOLD,
    }
...
Dicas de uso:
  • Escreva a ideia e termine com END em uma linha vazia se for longa
  • Informe o caminho de um arquivo: "Use a ideia em brainstorm.md"
  • Responda às perguntas de follow-up do agente para refinar o design
"""

def print_banner(system_prompt_path: Path) -> None:
    print()
    print(fmt(SEPARATOR, C.DIM))
    print(fmt("  AGENTE WORKING BACKWARDS DESIGNER", C.BOLD + C.CYAN))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Agente Working Backwards Designer interativo")
    parser.add_argument("--file", "-f", help="Arquivo com ideia bruta para carregar ao iniciar")
    ...
    if args.file:
        ...
        user_input = f"Por favor, use a ideia descrita no arquivo: {file_path.resolve()}"
        print(fmt(f"\n> {user_input}", C.BOLD))
        print(fmt("\n  Processando...", C.DIM))
    ...
    while True:
        print()
        try:
            user_input = read_multiline("Ideia/Feedback ›")
