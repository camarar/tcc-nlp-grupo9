"""
Agente Code Reviewer — Revisão técnica fullstack interativa via terminal.

Uso:
    py code_reviewer.py
    py code_reviewer.py --file caminho/para/arquivo.py
"""
...
PROMPT_FILE = SCRIPT_DIR / "Agente Code Reviewer.md"
META_FILE = SCRIPT_DIR / "code-reviewer.meta.json"

# Fonte única do modelo: code-reviewer.meta.json → model.api. Fallback só se o
# arquivo estiver ausente/corrompido, para o runner nunca quebrar por causa disso.
...
TOOLS = [
    {
        "name": "read_file",
        "description": (
            "Lê o conteúdo de um arquivo do sistema de arquivos local para revisão. "
            "Use quando o usuário fornecer um caminho de arquivo em vez de colar o código."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "list_directory",
        "description": "Lista arquivos de código em um diretório para ajudar o usuário a escolher o que revisar.",
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {
                ...
                "extensions": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Extensões para filtrar, ex: ['.py', '.ts']. Omita para listar tudo."
                }
            },
            "required": ["path"]
        }
...
def tool_list_directory(path: str, extensions: list[str] | None = None) -> str:
    try:
        ...
        CODE_EXTS = {
            ".py", ".ts", ".tsx", ".js", ".jsx", ".dart", ".go", ".rs",
            ".java", ".kt", ".swift", ".rb", ".php", ".cs", ".cpp", ".c",
            ".vue", ".svelte", ".html", ".css", ".scss", ".sql", ".yaml",
            ".yml", ".json", ".toml", ".tf"
        }
        filter_exts = {e.lower() for e in extensions} if extensions else CODE_EXTS
        ...
        if not files:
            return f"Nenhum arquivo de código encontrado em {path}"
...
def load_system_prompt() -> str:
    if not PROMPT_FILE.exists():
        ...
        return "Você é um revisor de código fullstack especializado."
...
def read_multiline(prompt_text: str) -> str:
    """
    Lê entrada do usuário. Para código multi-linha, o usuário digita
    END em uma linha vazia para finalizar. Uma linha única é enviada diretamente.
    """
    print(fmt(prompt_text, C.BOLD + C.CYAN))
    print(fmt("  (para código multi-linha, termine com END em uma linha vazia)", C.DIM))
    print()
    ...
    # Se parece ser texto curto (sem quebras de contexto), envia direto
    code_indicators = ("```", "def ", "function ", "class ", "import ", "from ",
                       "const ", "let ", "var ", "public ", "private ", "void ")
    if not any(first.startswith(ind) for ind in code_indicators) and len(first) < 200:
...
def print_response(text: str) -> None:
    ...
    print(fmt("  REVISÃO", C.BOLD + C.GREEN))
    print(fmt(SEPARATOR, C.DIM))
    print()
    ...
    tag_colors = {
        ...
        "[ALTO]":    C.YELLOW + C.BOLD,
        "[MÉDIO]":   C.CYAN + C.BOLD,
        "[BAIXO]":   C.WHITE,
        "[ELOGIO]":  C.GREEN + C.BOLD,
    }
...
Dicas de uso:
  • Cole o código diretamente e termine com END em uma linha vazia
  • Informe o caminho de um arquivo: "Revise o arquivo src/auth.ts"
  • Informe um diretório: "Liste os arquivos em src/services"
  • Faça perguntas de follow-up sem precisar colar o código novamente
"""

def print_banner(system_prompt_path: Path) -> None:
    print()
    print(fmt(SEPARATOR, C.DIM))
    print(fmt("  AGENTE CODE REVIEWER", C.BOLD + C.CYAN))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Agente Code Reviewer interativo")
    parser.add_argument("--file", "-f", help="Arquivo para revisar diretamente ao iniciar")
    ...
    # Revisão direta via --file
    if args.file:
        file_path = Path(args.file)
        user_input = f"Por favor, revise o arquivo: {file_path.resolve()}"
        print(fmt(f"\n> {user_input}", C.BOLD))
        print(fmt("\n  Processando...", C.DIM))
